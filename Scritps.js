...
const lessons = [
    ...
    // Añade más niveles aquí con locked: true
    {
        id: 3,
        title: "Bucles 'for'",
        completed: false,
...
function loadProgress() {
    ...
    progressData.forEach(savedLesson => {
        ...
        if (lesson) {
            ...
            // Asegura que el estado de bloqueo sea el correcto
...
function startGame() {
    ...
    ui.welcomeScreen.style.pointerEvents = 'none';

    // Muestra el contenedor principal
    ui.gameContainer.style.opacity = '1';
    ui.gameContainer.style.pointerEvents = 'auto';

    // Inicia la carga del progreso y la lección
    loadProgress();
...
// Init: El juego se inicializa al hacer clic en "Continuar al Juego" (ver startGame).
// Solo cargamos el progreso, pero la UI se mantiene en la pantalla de bienvenida.
document.addEventListener('DOMContentLoaded', () => {
    // Si el usuario ya completó el primer nivel, podría saltar la bienvenida
    loadProgress();
    if (lessons.some(l => l.completed)) {
        startGame();
    } else {
        // Asegura que el game container esté oculto si es la primera vez
        ui.gameContainer.style.opacity = '0';
        ui.gameContainer.style.pointerEvents = 'none';
        ui.welcomeScreen.style.opacity = '1';
        ui.welcomeScreen.style.pointerEvents = 'auto';
    }
});
